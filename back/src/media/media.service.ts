...
export class MediaService {
  ...
  async uploadMediaService(id: string, file: Express.Multer.File) {
    ...
    if(!profile) throw new NotFoundException(`Proveedor con Id ${id} no encontrado`)
  ...
  async deleteUserPofileService(id: string) {
    const user = await this.usersRepository.findOne({where: {id: id}})
    if(!user) throw new NotFoundException(`Usuario con Id ${id} no encontrado`)
    if(!user.imgUrl) throw new BadRequestException(`El usuario no tiene imagen de perfil para eliminar`)
